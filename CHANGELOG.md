...
- Catchup（直近N分の要約）で**任意の分数を指定できる**ようにした。ボタンは 3/5/10 分に整理し（1分は短すぎて使われていなかった。1分の発話は約570字で、「一文サマリ+箇条書き3〜6点」に要約する意味がほとんど無い）、隣の入力欄に 1〜30 の整数を打って Enter（または「要約」）で実行する。上限は30分——発話速度の実測（中央値571字/分）で約17,000字・1回 $0.0072 なのでコストは制約にならないが、出力形式が固定なので30分より広い範囲を要約しても情報が薄まるだけで「離席から戻って追いつく」用途を満たさない（会議全体の把握は上の「全体像」の役割）。範囲外・非整数の入力は**丸めずに弾く**（30分で勝手に実行されると、なぜその範囲になったのか分からなくなるため）。全角数字と前後の空白は受け付ける
...
- Catchup の入力に上限（24,000字・末尾優先）を設け、タイムアウトを20秒→40秒に延ばした。これまで Catchup は対象期間の発話を無制限に送っていた（ボタンが最大10分＝約5,700字だったので実害が出ていなかっただけ）。任意入力で30分を指定できるようにすると素で約17,000字が飛ぶため、上限30分の発話が実測速度でクリップされずに通る幅を確保した上で、早口・多人数で伸びた場合は古い側から削る（「直近N分に追いつく」用途なので残すのは直近側）。タイムアウトは日本語対訳を20→40秒にしたのと同じ理由——応答は通常数秒だが20秒はネットワークの一時的な詰まりを吸収しきれず、落ちると「要約の生成に失敗しました」カードが残って押し直しになる

- **日本語対訳が出るまでの待ち時間を約半分にした**（実測の内訳から見積もって 平均10秒 → 約5秒）。2026-08-14 の講義（対訳の生成604回）を実測したところ、遅延の内訳は「まとめて送るための待ち 最大8秒・平均4秒」＋「AIの応答 6.0秒」だった。1回あたり平均1.52件しか溜まっておらず**毎回タイマー待ちで発火**していたため、待ちを8秒→3秒に短縮。あわせて**既定では整形をやめて日本語訳だけを生成する**ようにした（AIの出力が84→48トークンに減り、応答が中央値 1.59秒 → 1.24秒、1回あたりの利用料も **-29%**）。副次的な効果として、整形しない分**文字起こし本文がAIに書き換えられなくなる**（英語の講義では `um`/`uh` が書き起こしの時点で落ちていて整形の仕事がほぼ無い一方、"In my history" → "In my experience" のような原文の書き換えが実際に起きていた）。日本語の会議ではフィラー除去・言い直しの統合が効くため、従来動作に戻せる: `defaults write com.meetscribe.app cleanerMode format-translate`（環境変数 `MEETSCRIBE_CLEANER_MODE=format-translate` も可。ただしFinder/Dockから起動した場合は環境変数が届かないため、恒久設定は前者を使う）
...
- 上記のマイク自動再起動が、再起動に失敗した直後に自分の警告バナーを消して以降の検知を止めてしまう問題を修正。音声が届いていないのに画面上は「録音中」・エラー表示なしに戻り、**警告が出たまま気付ける状態すら失われていた**。復帰の判定を「実際にマイクから音声が届いた」ときだけに限定し、届かないまま再起動が続く場合は上限回数まで再試行して警告を残す。あわせて音声が途絶えたらマイクのレベル表示を0に落とす（途絶時の値で止まったままだと「録れている」ように見え、10分無音での自動保存も妨げていた）
- マイクの起動が途中で失敗した後、録音を開始し直すとアプリが即終了する問題を修正。失敗時に音声取り出し口（tap）とVoice Processing（エコー除去）が解放されずに残り、次の開始で二重登録になっていた（Mac全体のオーディオが固まる要因にもなり得た）。あわせて入力デバイスが無い状態での開始は、クラッシュではなくバナー付きのエラーで返すようにした
