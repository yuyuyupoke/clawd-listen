...
- **Copilot パネル** — ボタン押下（3/5/10分）または任意の分数（1〜30分）の入力で直近N分の発話を日本語で要約する「Catchup」と、発話量・経過時間トリガーで自動更新される「全体像」（目的・議題・現在地）。
...
右パネルは録音中、自動で「全体像」（目的・議題・現在地）を更新し続ける。加えて下部の 3/5/10 分ボタンを押すか、隣の入力欄に分数（1〜30）を打って Enter すると、その期間の発話を日本語で要約した「Catchup」カードが追加される。

| 操作 | 挙動 |
|---|---|
| 何もしない | 発話が一定量溜まる、または一定時間経過するたびに全体像が自動更新される |
| Catchup ボタン（3/5/10分） | 押した時点から遡ったN分間の発話を要約してカード表示 |
| Catchup 分数入力（1〜30分） | 任意の分数を入力して Enter（または「要約」）で同じ要約を実行。範囲外・非整数は実行されず、入力欄の枠が赤くなる（理由はツールチップに表示） |
